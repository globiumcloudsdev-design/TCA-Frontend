...
import { CheckSquare, X, Minus, QrCode, Search, Filter, FileText, Users, UserSquare, Eye, PieChart, ArrowLeft } from 'lucide-react';
...
export default function AttendancePage({ type }) {
  ...
  const sections = useMemo(() => {
    if (!filters.class_id || filters.class_id === 'all' || !classesData?.data) return [];
  ...
  const { data: attendanceData, isLoading, refetch } = useQuery({
    queryKey: ['attendance', { ...filters, section_id: filters.section_id || undefined }, page, pageSize],
...
// ─────────────────────────────────────────────────────────────────────────────
function AttendanceReportGenerator({ terms, yearsData, classesData, currentYearId, onClose }) {
  ...
  const sections = useMemo(() => {
    ...
    const selectedClass = classesData.data.find(c => c.id === reportFilters.class_id);
    return selectedClass?.sections || [];
      ...
      <div className="space-y-6 animate-in slide-in-from-right-4 duration-300 py-2">
        <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
           <div>
             ...
             <p className="text-sm text-slate-500 mt-1">
               {reportType === 'class' ? 'Class-wide' : 'Individual'} Attendance Summary
             </p>
           </div>
           <Button variant="outline" size="sm" onClick={() => setReportData(null)} className="h-9 shrink-0 shadow-sm rounded-xl px-4 font-semibold text-slate-600 dark:text-slate-300">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
           ...
           <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-4 flex flex-col justify-center items-center hover:scale-[1.02] transition-transform">
             <span className="text-4xl font-black text-emerald-600 dark:text-emerald-400">{reportData.present}</span>
             <span className="text-[10px] uppercase font-bold text-emerald-700/60 dark:text-emerald-400/60 tracking-wider mt-1">Present</span>
           </div>
           {/* Absent */}
           <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-2xl p-4 flex flex-col justify-center items-center hover:scale-[1.02] transition-transform">
             <span className="text-4xl font-black text-red-600 dark:text-red-400">{reportData.absent}</span>
             <span className="text-[10px] uppercase font-bold text-red-700/60 dark:text-red-400/60 tracking-wider mt-1">Absent</span>
           </div>
           {/* Late */}
           <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-2xl p-4 flex flex-col justify-center items-center hover:scale-[1.02] transition-transform">
             <span className="text-4xl font-black text-amber-600 dark:text-amber-400">{reportData.late}</span>
             <span className="text-[10px] uppercase font-bold text-amber-700/60 dark:text-amber-400/60 tracking-wider mt-1">Late</span>
           </div>
           {/* Holiday */}
           <div className="bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-200 dark:border-indigo-800 rounded-2xl p-4 flex flex-col justify-center items-center hover:scale-[1.02] transition-transform">
             <span className="text-4xl font-black text-indigo-600 dark:text-indigo-400">{reportData.holiday}</span>
             <span className="text-[10px] uppercase font-bold text-indigo-700/60 dark:text-indigo-400/60 tracking-wider mt-1">Holiday</span>
           </div>
        </div>
        ...
        <div className="relative border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-6 sm:p-8 rounded-3xl overflow-hidden shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6 group hover:border-indigo-200 dark:hover:border-indigo-900/50 transition-colors">
           ...
           <div className="relative z-10 flex flex-col gap-1">
             <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 mb-1">
                ...
                <h4 className="font-bold uppercase tracking-widest text-xs">Performance Rate</h4>
             </div>
             ...
             <p className="text-[11px] font-semibold text-slate-500 mt-2 uppercase tracking-wide">Calculated System Accuracy</p>
           </div>
           
           <div className="relative z-10 w-full sm:max-w-xs flex-shrink-0 pt-2 sm:pt-0">
             <div className="flex justify-between items-end mb-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Target 100%</span>
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end pt-4 gap-3 border-t border-slate-100 dark:border-slate-800/50">
           <Button variant="ghost" onClick={onClose} className="rounded-xl font-semibold">Close</Button>
           <Button onClick={handleGenerate} disabled={isGenerating} className="px-6 h-12 bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-600/20 rounded-xl font-bold">
  ...
  return (
    <div className="space-y-8 py-2">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          <div className={`p-3 rounded-full ${reportType === 'class' ? 'bg-indigo-100 dark:bg-indigo-900/50' : 'bg-slate-100 dark:bg-slate-900'}`}>
            <Users className={`w-6 h-6 ${reportType === 'class' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}`} />
          </div>
          <div>
            <h4 className="font-bold text-sm">Class-wise Report</h4>
            <p className="text-xs opacity-70 mt-0.5">Summary of attendance for an entire class</p>
          </div>
        </div>
        ...
        >
          <div className={`p-3 rounded-full ${reportType === 'student' ? 'bg-indigo-100 dark:bg-indigo-900/50' : 'bg-slate-100 dark:bg-slate-900'}`}>
            <UserSquare className={`w-6 h-6 ${reportType === 'student' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}`} />
          </div>
          <div>
            <h4 className="font-bold text-sm">Student-wise Report</h4>
            <p className="text-xs opacity-70 mt-0.5">Detailed attendance history of a student</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <SelectField
            ...
            onChange={(val) => setReportFilters(p => ({...p, academic_year_id: val}))}
            placeholder="Select Year"
          />

          {timeframe === 'monthly' && (
            <div className="space-y-1.5 focus-within:text-primary">
               <label className="text-sm font-semibold tracking-tight text-slate-700 dark:text-slate-300">Reporting Month</label>
               <Input 
                 type="month" 
          ...
          {reportType === 'class' ? (
            <>
              <SelectField
                ...
                onChange={(val) => setReportFilters(p => ({...p, class_id: val, section_id: ''}))}
                placeholder={`Select ${terms?.class || "Class"}`}
              />
              <SelectField
                ...
                onChange={(val) => setReportFilters(p => ({...p, section_id: val}))}
                placeholder="All Sections"
             ...
             <div className="md:col-span-2 space-y-1.5 focus-within:text-primary">
                ...
                <p className="text-[11px] text-slate-400">Search to select a specific student for this report.</p>
             </div>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between pt-4 gap-4">
        <p className="text-xs text-slate-500 font-medium">Select criteria to preview or generate the attendance report.</p>
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto shrink-0">
          <Button variant="ghost" className="rounded-xl font-semibold h-11" onClick={onClose}>Cancel</Button>
          ...
          >
            {isViewing ? (
              <><div className="w-4 h-4 mr-2 border-2 border-indigo-500/30 border-t-indigo-500 rounded-full animate-spin" /> Loading...</>
            ) : (
          ...
          >
            {isGenerating ? (
               <><div className="w-4 h-4 mr-2 border-2 border-white/30 border-t-white dark:border-slate-900/30 dark:border-t-slate-900 rounded-full animate-spin" /> Preparing...</>
            ) : (
