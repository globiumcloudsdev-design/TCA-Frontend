'use client';
/**
 * FacultiesPage — University faculty management
 */
...
import { DUMMY_FACULTIES } from '@/data/dummyData';
...
export default function FacultiesPage({ type }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['faculties', type, page, pageSize, search, status],
    queryFn: async () => {
      try { const { facultyService } = await import('@/services'); return await facultyService.getAll({ page, limit: pageSize, search, status }); }
      catch {
        const d = DUMMY_FACULTIES.filter(r => (!search || r.name.toLowerCase().includes(search.toLowerCase())) && (!status || r.status === status));
  ...
  const rows = data?.data?.rows ?? DUMMY_FACULTIES;
  const total = data?.data?.total ?? rows.length;
  const totalPages = data?.data?.totalPages ?? 1;

  const save = useMutation({
    mutationFn: async (vals) => {
      try { const { facultyService } = await import('@/services'); return editing ? await facultyService.update(editing.id, vals) : await facultyService.create(vals); }
      catch { return { data: vals }; }
    },
    onSuccess: () => { toast.success(editing ? 'Updated' : 'Created'); qc.invalidateQueries({ queryKey: ['faculties'] }); closeModal(); },
    onError: () => toast.error('Save failed'),
  });

  const remove = useMutation({
    mutationFn: async (id) => {
      try { const { facultyService } = await import('@/services'); return await facultyService.delete(id); }
      catch { return { success: true }; }
    },
    onSuccess: () => { toast.success('Deleted'); qc.invalidateQueries({ queryKey: ['faculties'] }); setDeleting(null); },
  ...
  const columns = useMemo(() => [
    { accessorKey: 'name', header: 'Faculty', cell: ({ row: { original: r } }) => <div><p className="font-medium">{r.name}</p><p className="text-xs text-muted-foreground font-mono">{r.code}</p></div> },
    { accessorKey: 'dean',        header: 'Dean',         cell: ({ getValue }) => getValue() || '—' },
    { accessorKey: 'departments', header: 'Departments',  cell: ({ getValue }) => getValue() ?? 0 },
    { accessorKey: 'programs',    header: 'Programs',     cell: ({ getValue }) => getValue() ?? 0 },
    { accessorKey: 'staff',       header: 'Staff',        cell: ({ getValue }) => getValue() ?? 0 },
    ...
    { id: 'actions', header: 'Actions', enableHiding: false, cell: ({ row }) => (
      <div className="flex items-center justify-end gap-1">
        {canDo('faculty.update') && <button onClick={() => openEdit(row.original)} className="rounded p-1.5 hover:bg-accent" title="Edit"><Pencil size={13} /></button>}
        {canDo('faculty.delete') && <button onClick={() => setDeleting(row.original)} className="rounded p-1.5 text-destructive hover:bg-destructive/10" title="Delete"><Trash2 size={13} /></button>}
      </div>
    )},
  ], [canDo]);
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Faculties" description={`${total} faculties`} />
      <div className="grid gap-4 sm:grid-cols-3">
        <StatsCard label="Total Faculties" value={total}                                         icon={<Building2 size={18} />} />
        <StatsCard label="Total Programs"  value={rows.reduce((s, r) => s + (r.programs ?? 0), 0)}   icon={<Building2 size={18} />} />
        <StatsCard label="Total Staff"     value={rows.reduce((s, r) => s + (r.staff ?? 0), 0)}      icon={<Building2 size={18} />} />
      </div>
      <DataTable columns={columns} data={rows} loading={isLoading} emptyMessage="No faculties found"
        search={search} onSearch={(v) => { setSearch(v); setPage(1); }} searchPlaceholder="Search faculties…"
        filters={[{ name:'status', label:'Status', value:status, onChange:(v) => { setStatus(v); setPage(1); }, options:STATUS_OPTS }]}
        action={canDo('faculty.create') ? <button onClick={openAdd} className="flex items-center gap-1.5 rounded-md bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90"><Plus size={14} /> New Faculty</button> : null}
        enableColumnVisibility
        exportConfig={{ fileName: 'faculties' }}
        pagination={{ page, totalPages, onPageChange: setPage, total, pageSize, onPageSizeChange: (s) => { setPageSize(s); setPage(1); } }} />

      <AppModal open={modal} onClose={closeModal} title={editing ? 'Edit Faculty' : 'New Faculty'} size="md"
      ...
      {/* Delete Confirm */}
      <AppModal open={!!deleting} onClose={() => setDeleting(null)} title="Delete Faculty" size="sm"
        footer={
          <>
        ...
        <p className="text-sm text-muted-foreground">Delete faculty <strong>{deleting?.name}</strong>? This cannot be undone.</p>
      </AppModal>
    </div>
  );
}
