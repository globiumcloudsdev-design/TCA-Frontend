      ...
      <AppModal open={modal} onClose={closeModal} title={editing ? 'Edit Department' : 'New Department'} size="md"
        ...
        <form id="dept-form" onSubmit={handleSubmit((v) => save.mutate(v))} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5"><label className="text-sm font-medium">Department Name <span className="ml-0.5 text-destructive font-semibold">*</span></label><input {...register('name')} className="input-base" />{errors.name && <p className="text-xs text-destructive">{errors.name.message}</p>}</div>
