...
export const payrollService = {
  ...
  // In frontend/src/services/payrollService.js
getYears: async () => {
  try {
    const response = await api.get('/payroll/years');
    return response.data;
  } catch (error) {
  ...
  generatePayroll: (payload) => api.post('/payroll/generate', payload).then(r => r.data),

  // Update payslip (status, payment method, remarks)
  update: (id, data) => api.patch(`/payroll/${id}`, data).then(r => r.data),

  // Delete payslip
  delete: (id) => api.delete(`/payroll/${id}`).then(r => r.data),
};