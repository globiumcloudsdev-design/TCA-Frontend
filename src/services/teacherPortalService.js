...
export const teacherPortalService = {
  ...
  updateProfile: (data) =>
    api
      .put("/portal/teacher/profile", data, {
  ...
  getClassDetails: (classId) =>
    api.get(`/portal/teacher/classes/${classId}`).then(unwrap),

  // Students
  getMyStudents: (filters = {}, page = 1, limit = 20) =>
    api
      .get("/portal/teacher/students", { params: { ...filters, page, limit } })
      .then(unwrap),
  getStudentDetails: (studentId) =>
    api.get(`/portal/teacher/students/${studentId}`).then(unwrap),
  ...
  createAssignment: (formData) =>
    api
      .post("/portal/teacher/assignments", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then(unwrap),
  ...
  updateAssignment: (assignmentId, formData) =>
    api
      .put(`/portal/teacher/assignments/${assignmentId}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then(unwrap),
  ...
  gradeSubmission: (submissionId, data) =>
    api
      .post(`/portal/teacher/submissions/${submissionId}/grade`, data)
      .then(unwrap),
  ...
  // Notices
  getNotices: (limit = 10) =>
    api.get("/portal/teacher/notices", { params: { limit } }).then(unwrap),
  ...
  getTeacherExams: (filters = {}, page = 1, limit = 10) =>
    api
      .get("/portal/teacher/exams", { params: { ...filters, page, limit } })
      .then(unwrap),
  ...
  getExamResults: (examId, filters = {}, page = 1, limit = 20) =>
    api
      .get(`/portal/teacher/exams/${examId}/results`, {
        params: { ...filters, page, limit },
      })
      .then(unwrap),
  ...
  addExamResults: (examId, results) =>
    api
      .post(`/portal/teacher/exams/${examId}/results`, { results })
      .then(unwrap),
  ...
  getExamEntryStudents: (examId, filters = {}, page = 1, limit = 100) =>
    api
      .get(`/portal/teacher/exams/${examId}/entry-students`, {
        params: { ...filters, page, limit },
      })
      .then(unwrap),
  ...
  getLeaveRequests: (filters = {}, page = 1, limit = 10) =>
    api
      .get("/portal/teacher/leave-requests", {
        params: { ...filters, page, limit },
      })
      .then(unwrap),
  ...
  getLeaveBalance: () => api.get("/portal/teacher/leave-balance").then(unwrap),

  getPendingLeaveApprovals: (filters = {}, page = 1, limit = 10) =>
    api
      .get("/portal/teacher/leave-requests/pending", {
        params: { ...filters, page, limit },
      })
      .then(unwrap),
