...
export const POLICY_CONFIG_TEMPLATES = {
  [POLICY_TYPES.ID_CARD]: {
    ...
    show_photo: true,
    show_watermark: true,
    qr_enabled: true,                   // ✅ default true, toggle removed
    school_info: {
      name: 'Green Valley High School',
      tagline: 'Excellence in Education',
      logo_url: null,
  ...
  [POLICY_TYPES.PAYROLL]: {
    // Updated attendance rules as per requirement
    attendance_rules: {
      late_arrival: {
        enabled: true,
        threshold_count: 3,         // 3 late arrivals in a month
        deduction_type: 'daily_salary',
        deduction_value: 1          // deduct 1 full day salary
      },
      full_day_absent: {
        enabled: true,
        deduction_type: 'daily_salary',
...
export const getFieldsByCardType = (cardType) => {
  if (cardType === 'staff') return STAFF_ID_FIELDS;
  return STUDENT_ID_FIELDS;
};
