...
const ROLE_ICONS = {
  ...
  TEACHER: { icon: Briefcase, color: 'bg-tca-primary/10 text-tca-primary', label: 'Teacher' },
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 relative overflow-hidden">
      {/* Background Glows */}
      <div className="absolute top-[-10%] left-[-10%] w-[80%] sm:w-[40%] h-[40%] rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[80%] sm:w-[40%] bg-violet-600/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="w-full max-w-lg relative z-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
          {PORTAL_BUTTONS.map((portal) => {
            ...
            return (
              ...
              >
                <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl ${portal.bg} flex items-center justify-center group-hover:scale-110 transition-transform`}>
                  <Icon className={`w-4 h-4 sm:w-5 sm:h-5 ${portal.text}`} />
                </div>
        ...
        {/* Login Card */}
        <div className="bg-slate-900/60 backdrop-blur-2xl border border-white/10 rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden">
          <Tabs defaultValue="single" onValueChange={setLoginMode} className="w-full">
            <TabsList className="w-full h-12 sm:h-14 bg-white/5 p-1 rounded-none border-b border-white/5">
              <TabsTrigger value="single" className="flex-1 rounded-xl sm:rounded-2xl data-[state=active]:bg-tca-primary data-[state=active]:text-white text-slate-400 text-xs sm:text-sm font-bold transition-all">
                <UserCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2" /> Single
              </TabsTrigger>
              <TabsTrigger value="dual" className="flex-1 rounded-xl sm:rounded-2xl data-[state=active]:bg-tca-primary data-[state=active]:text-white text-slate-400 text-xs sm:text-sm font-bold transition-all">
                <Users2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2" /> Dual
              </TabsTrigger>
            </TabsList>

            <div className="p-6 sm:p-8">
              <TabsContent value="single" className="mt-0">
                <form onSubmit={handleSubmitSingle(onSingleLogin)} className="space-y-4 sm:space-y-5">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className="h-12 sm:h-13 bg-white/5 border-white/10 text-white pl-11 focus:ring-tca-primary rounded-xl sm:rounded-2xl"
                        disabled={loading}
                      />
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between ml-1">
                      ...
                      <Link href="/forgot-password" size="sm" className="text-[10px] font-bold text-tca-primary hover:text-indigo-300 transition-colors uppercase">Forgot?</Link>
                    </div>
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className="h-12 sm:h-13 bg-white/5 border-white/10 text-white pl-11 pr-11 focus:ring-tca-primary rounded-xl sm:rounded-2xl"
                        disabled={loading}
                      />
                  ...
                  <Button type="submit" disabled={loading} className="w-full h-14 bg-tca-primary hover:bg-tca-primary text-white font-black rounded-2xl shadow-xl shadow-tca-primary/20 transition-all active:scale-95 text-base">
              ...
              <TabsContent value="dual" className="mt-0 text-white">
                <form onSubmit={handleSubmitDual(onDualLogin)} className="space-y-4 sm:space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className="h-12 sm:h-13 bg-white/5 border-white/10 text-white pl-11 rounded-xl sm:rounded-2xl"
                        disabled={loading}
                      />
                    </div>
                    {errorsDual.email && <p className="text-[10px] text-rose-400 font-bold ml-1 uppercase">{errorsDual.email.message}</p>}
                  </div>
                  <div className="bg-tca-primary/5 border border-tca-primary/10 p-4 sm:p-5 rounded-2xl flex items-start gap-3 sm:gap-4">
                    <Sparkles className="w-5 h-5 text-tca-primary mt-0.5 shrink-0" />
                  ...
                  <Button type="submit" disabled={loading} className="w-full h-14 bg-tca-primary hover:bg-tca-primary text-white font-black rounded-2xl transition-all shadow-xl shadow-tca-primary/20">
                    {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Continue to Selection"}
                  </Button>
                </form>
              </TabsContent>
            </div>
        ...
        <DialogContent className="w-[95vw] sm:max-w-sm bg-slate-900 border-white/10 text-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8">
          ...
          <div className="space-y-5 sm:space-y-6 py-4">
            <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-white/5 rounded-2xl border border-white/5">
              <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl ${ROLE_ICONS[selectedAccount?.user_type]?.color || 'bg-slate-800'} flex items-center justify-center shrink-0 shadow-lg`}>
                {(() => { const IconComp = ROLE_ICONS[selectedAccount?.user_type]?.icon || UserCircle; return <IconComp size={20} />; })()}
              </div>
              <div className="min-w-0">
                <p className="text-sm sm:text-base font-bold text-white truncate">{selectedAccount?.display_name}</p>
                <p className="text-[9px] sm:text-[10px] font-bold text-tca-primary uppercase tracking-widest">{selectedAccount?.display_role}</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  placeholder="••••••••"
                  className="h-11 sm:h-12 bg-white/5 border-white/10 text-white pr-10 focus:border-tca-primary rounded-xl"
            ...
            <Button onClick={handlePasswordSubmit} disabled={!accountPassword || loading} className="w-full h-12 bg-tca-primary hover:bg-tca-primary text-white font-black rounded-xl transition-all shadow-lg shadow-tca-primary/20">
