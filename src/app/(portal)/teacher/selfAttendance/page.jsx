...
// import LeaveRequestForm from "@/components/portal/teacher/LeaveRequestForm";
...
//         description="Submit your leave request with start and end dates. Your leave balance is shown below."
//         size="md"
//       >
//         {leaveBalance ? (
//           <LeaveRequestForm 
...
//             <p className="text-xs text-slate-500">Loading leave balance...</p>
//           </div>
//         )}
//       </AppModal>
//     </div>
...
const formatDateRange = (fromDate, toDate) => {
  ...
  if (fromDate === toDate) return format(parseISO(fromDate), "dd MMM yyyy");
  return `${format(parseISO(fromDate), "dd MMM")} - ${format(parseISO(toDate), "dd MMM yyyy")}`;
};
...
export default function TeacherSelfAttendancePage() {
  ...
  // New state for leave requests list
  const [leaveRequests, setLeaveRequests] = useState([]);
  const [leaveRequestsLoading, setLeaveRequestsLoading] = useState(false);
  const [leaveRequestsPagination, setLeaveRequestsPagination] = useState({ page: 1, limit: 10, totalPages: 1 });
  const [leaveRequestsFilters, setLeaveRequestsFilters] = useState({ status: "" });
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          return <span className="text-xs">{formatDateRange(from_date, to_date)}</span>;
        },
      },
      {
        accessorKey: "leaveType.leave_type_name",
        header: "Type",
        cell: ({ row }) => (
          <span className="text-xs font-medium">
            {row.original.leaveType?.leave_type_name || "—"}
          </span>
        ),
      },
      {
        ...
        header: "Status",
        cell: ({ row }) => (
          <Badge className={getLeaveStatusColor(row.original.status)}>
            {row.original.status}
          </Badge>
        ),
      },
      {
        ...
        header: "Reason",
        cell: ({ row }) => (
          <span className="text-xs max-w-[200px] truncate block" title={row.original.reason}>
            {row.original.reason || "—"}
          </span>
        ),
      },
      {
        accessorKey: "admin_remarks",
        header: "Admin Remarks",
        cell: ({ row }) => (
          <span className="text-xs text-muted-foreground block" title={row.original.admin_remarks}>
            {row.original.admin_remarks || "—"}
          </span>
        ),
      },
      {
  ...
  const fetchLeaveRequests = async (page = 1) => {
    try {
      ...
      const params = {
        page,
        limit: leaveRequestsPagination.limit,
      };
      if (leaveRequestsFilters.status) params.status = leaveRequestsFilters.status.toUpperCase();
      const res = await teacherPortalService.getLeaveRequests(params, page, leaveRequestsPagination.limit);
      setLeaveRequests(res.data || []);
      if (res.pagination) setLeaveRequestsPagination(res.pagination);
    } catch (error) {
  ...
  const handleCancelLeave = async () => {
    ...
    try {
      await teacherPortalService.cancelLeaveRequest(selectedLeaveRequest.id);
      toast.success("Leave request cancelled");
      fetchLeaveRequests(leaveRequestsPagination.page);
      fetchLeaveBalance(); // refresh balance if needed
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await teacherPortalService.getSelfAttendanceReport({ month, from_date, to_date });
      const actualData = res?.data || res;
      if (actualData?.stats) setReportStats(actualData.stats);
      if (actualData?.daily) setHistoryData(actualData.daily);
    } catch (error) {
      console.error("Failed to fetch report:", error);
      toast.error("Failed to fetch report stats");
    } finally {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      console.error(error);
      if (error?.response?.status === 409) toast.error("Already checked in today");
      else if (error?.response?.status === 403) toast.error("You have an approved leave today");
      else toast.error(error?.response?.data?.message || "Check-in failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      console.error(error);
      if (error?.response?.status === 409) toast.error("Already checked out today");
      else if (error?.response?.status === 400) toast.error("Check-in not found for today. Cannot check out.");
      else toast.error(error?.response?.data?.message || "Check-out failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getCountdownInfo = () => {
    ...
    try {
      ...
      if (diffStartMs > thresholdMs && diffStartMs > 0) {
        ...
        const m = Math.floor((totalSecs % 3600) / 60);
        const s = totalSecs % 60;
        const timeStr = [h > 0 ? `${h}h` : null, m > 0 || h > 0 ? `${m}m` : null, `${s}s`].filter(Boolean).join(" ");
        return { isEarly: true, isClosed: false, timeStr, shiftTime: shiftStartTime };
      }
      if (time > endShiftDate) return { isEarly: false, isClosed: true };
  ...
  const leaveColumns = useMemo(() => [
    { accessorKey: "from_date", header: "Date Range", cell: ({ row }) => formatDateRange(row.original.from_date, row.original.to_date) },
    { accessorKey: "leave_type", header: "Type", cell: ({ getValue }) => <span className="capitalize">{getValue()?.replace(/_/g, " ")}</span> },
    { accessorKey: "status", header: "Status", cell: ({ getValue }) => (
      <Badge className={getLeaveStatusColor(getValue())}>{getValue()}</Badge>
    ) },
    { accessorKey: "reason", header: "Reason", cell: ({ getValue }) => <span className="text-xs line-clamp-2">{getValue() || "—"}</span> },
    { accessorKey: "admin_remarks", header: "Admin Remarks", cell: ({ getValue }) => <span className="text-xs text-muted-foreground">{getValue() || "—"}</span> },
    { id: "actions", header: "Actions", cell: ({ row }) => (
      <div className="flex gap-1">
        {row.original.status === "pending" && (
          <Button variant="ghost" size="sm" onClick={() => { setSelectedLeaveRequest(row.original); setCancelModalOpen(true); }} className="h-7 text-rose-600">
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
            <div className="col-span-1 md:col-span-2 grid grid-cols-3 gap-4 border-r border-slate-100 pr-4">
              ...
              <div className="text-center p-3 bg-blue-50 rounded-xl border border-blue-100">
                <p className="text-[10px] uppercase font-bold text-blue-500 mb-1">Duration</p>
                <div className="flex flex-col items-center">
                  <p className="font-mono font-black text-blue-700">{getDurationDisplay()}</p>
                  {attendanceData?.check_in && !attendanceData?.check_out && (
                    <span className="text-[9px] font-bold text-blue-500 animate-pulse mt-0.5 w-max">🔴 LIVE TIMER</span>
                  )}
                </div>
            ...
            <div className="col-span-1 flex flex-col gap-3 justify-center items-center">
              <AnimatePresence mode="wait">
                {loadingInitial ? (
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-3 w-full max-w-[200px]">
                    <div className="relative w-full">
                      {countdown.isEarly && (
                         <div className="absolute -top-7 left-1/2 transform -translate-x-1/2 flex justify-center z-10 w-max">
                           <div className="bg-slate-800 text-white text-[10px] font-mono px-3 py-1 rounded-full shadow-lg border border-slate-700 animate-pulse">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-50">
           ...
           <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-200 shadow-sm">
             <MonthPicker value={reportMonth} onChange={(val) => setReportMonth(val)} />
             <Button onClick={handleDownloadPDF} disabled={reportLoading} className="h-8 text-xs bg-rose-600 hover:bg-rose-700 text-white rounded-lg px-4 shadow-sm font-bold transition-all flex items-center gap-1.5">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                Download PDF
             </Button>
           </div>
        ...
        {reportLoading ? (
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100/50">
                <p className="text-xs font-bold text-slate-500 mb-1">Present Days</p>
                <p className="text-2xl font-black text-emerald-600">{reportStats?.present ?? '--'} <span className="text-xs text-slate-400 font-medium">/ {reportStats?.total_days ?? '--'}</span></p>
              </div>
              <div className="bg-amber-50 rounded-xl p-4 border border-amber-100/50">
                <p className="text-xs font-bold text-slate-500 mb-1">Late Comings</p>
                <p className="text-2xl font-black text-amber-600">{reportStats?.late ?? '--'}</p>
      ...
      <Card className="border border-slate-200 shadow-sm rounded-2xl overflow-hidden bg-white">
        <CardHeader className="pb-4 border-b border-slate-50 bg-slate-50/30">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-sm font-bold text-slate-800">My Leave Requests</CardTitle>
            </div>
            <div className="flex items-center gap-2">
              <SelectField
                placeholder="All Status"
                value={leaveRequestsFilters.status}
                onChange={(val) => setLeaveRequestsFilters(prev => ({ ...prev, status: val }))}
                options={[
                ...
                className="w-32"
              />
              <Button variant="ghost" size="sm" onClick={() => fetchLeaveRequests(1)} className="h-8 w-8 p-0">
                <RefreshCw size={14} />
              </Button>
            </div>
          </div>
        ...
        <CardContent className="p-4 pt-4">
          <DataTable
            ...
            onPageChange={fetchLeaveRequests}
            noDataText="No leave requests found."
          />
        </CardContent>
      </Card>

      {/* Leave Application Modal (unchanged) */}
      <AppModal 
        open={leaveModalOpen}
        onClose={() => setLeaveModalOpen(false)}
        title="Leave Request Form"
      ...
      <AppModal
        ...
        onClose={() => { setCancelModalOpen(false); setSelectedLeaveRequest(null); }}
        title="Cancel Leave Request"
        size="sm"
        footer={
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setCancelModalOpen(false)}>No, Keep</Button>
            <Button variant="destructive" onClick={handleCancelLeave}>Yes, Cancel</Button>
          </div>
        }
      >
        <p className="text-sm">Are you sure you want to cancel this leave request?</p>
        {selectedLeaveRequest && (
          <p className="text-xs text-muted-foreground mt-2">
            {formatDateRange(selectedLeaveRequest.from_date, selectedLeaveRequest.to_date)} · {selectedLeaveRequest.leave_type}
          </p>
        )}
      </AppModal>
    </div>
