...
import { Badge } from '@/components/ui/badge';
import PageLoader from '@/components/common/PageLoader';
import PrintableResultCard from '@/components/cards/ResultCard';
...
const isSubjectPass = (marksObtained, totalMarks) => {
  ...
  const passingMarks = totalMarks * 0.4; // 40% passing threshold
  return marksObtained >= passingMarks;
};

const determineOverallStatus = (result, examPassPercentage = 40) => {
  // Check if any subject failed based on marks (not just grade)
  const hasFailedSubject = result.subjects?.some(sub => 
    !isSubjectPass(sub.marks_obtained, sub.total_marks)
  );
  if (hasFailedSubject) return 'fail';
  ...
  // Absent handling (if present)
  if (result.status === 'absent') return 'absent';
  
  return 'fail';
};

// Transform result with computed fields
const processResult = (result) => {
  const subjects = (result.subjects || []).map(sub => {
    const passed = isSubjectPass(sub.marks_obtained, sub.total_marks);
  ...
  const status = determineOverallStatus({ ...result, subjects });
...
// Transform for printable card using processed data
const transformForPrintableCard = (processedResult, child, institute) => {
  const nameParts = child.name?.split(' ') || [];
  const student = {
    first_name: nameParts[0] || child.name || '',
    last_name: nameParts.slice(1).join(' ') || '',
  ...
  const exam = {
    name: processedResult.exam_name,
    total_marks: processedResult.total_marks,
    subject_schedules: processedResult.subjects.map(sub => ({
  ...
  const cardResult = {
    subject_marks: processedResult.subjects.map(sub => ({
      subject_id: sub.subject_id,
      subject_name: sub.subject_name,
      marks_obtained: sub.marks_obtained,
      total_marks: sub.total_marks,
      grade: sub.grade,
      remarks: sub.remarks || (sub.passed ? 'Pass' : 'Fail'),
    })),
    total_marks_obtained: processedResult.obtained_marks,
    percentage: processedResult.overallPercentage,
    grade: processedResult.overallGrade,
...
const ResultDetailsModal = ({ result, isOpen, onClose }) => {
  ...
  return (
    <AppModal
      open={isOpen}
      onClose={onClose}
      title={processed.exam_name}
      description={`${format(new Date(processed.exam_date), 'dd MMM yyyy')} • ${processed.exam_type}`}
      size="xl"
    >
      <div className="space-y-5">
        {/* Score Overview */}
        <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-5 text-white">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm opacity-90">Overall Score</p>
              <p className="text-3xl font-bold">{processed.overallPercentage}%</p>
              <p className="text-sm opacity-90 mt-1">Grade: {processed.overallGrade}</p>
            </div>
            <div className="text-center">
              <p className="text-sm opacity-90">Rank</p>
              <p className="text-2xl font-bold">{processed.rank || '-'}</p>
              <p className="text-xs opacity-90">out of {processed.total_students || '-'}</p>
            </div>
            <div className="text-center">
              <p className="text-sm opacity-90">Marks</p>
              <p className="text-2xl font-bold">{processed.obtained_marks}</p>
              <p className="text-xs opacity-90">/ {processed.total_marks}</p>
            </div>
          </div>
        </div>
        ...
        {activeTab === 'subjects' && (
          <div className="space-y-4">
            {processed.subjects.map((subject, idx) => {
        ...
        {activeTab === 'analysis' && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-emerald-50 rounded-xl p-4 text-center">
                ...
                <p className="text-2xl font-bold text-emerald-600">
                  {processed.subjects.filter(s => s.passed).length}
                </p>
              </div>
              <div className="bg-red-50 rounded-xl p-4 text-center">
                <p className="text-xs text-slate-500">Failed Subjects</p>
            ...
            {processed.class_average && (
              <div className="bg-slate-50 rounded-xl p-4">
                <p className="text-sm font-semibold text-slate-700 mb-2">Class Performance Comparison</p>
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-xs text-slate-500">Your Score</p>
                    <p className="text-lg font-bold text-indigo-600">{processed.overallPercentage}%</p>
                  </div>
                  <div className="flex-1 mx-4">
                    <div className="relative pt-1">
                      <div className="flex mb-2 items-center justify-between">
                        <div className="text-xs text-slate-500">Class Average: {Math.round(processed.class_average)}%</div>
                      </div>
                      <div className="overflow-hidden h-2 text-xs flex rounded bg-slate-200">
                        <div
                          style={{ width: `${processed.class_average}%` }}
                          className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-slate-500"
                        />
                      </div>
                    </div>
                  ...
                  <div>
                    <p className="text-xs text-slate-500">Difference</p>
                    <p className={`text-lg font-bold ${processed.performance_compared_to_class >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                      {processed.performance_compared_to_class >= 0 ? '+' : ''}{Math.round(processed.performance_compared_to_class)}%
                    </p>
                  </div>
                </div>
              </div>
        ...
        {activeTab === 'feedback' && (
          <div className="space-y-4">
            {processed.remarks && (
              <div className="bg-indigo-50 rounded-xl p-4">
                <p className="text-sm font-semibold text-indigo-800 mb-2">Teacher's Remarks</p>
                <p className="text-sm text-indigo-700">{processed.remarks}</p>
              </div>
            )}
            {processed.teacher_feedback && (
              <div className="bg-amber-50 rounded-xl p-4">
                <p className="text-sm font-semibold text-amber-800 mb-2">Detailed Feedback</p>
                <p className="text-sm text-amber-700">{processed.teacher_feedback}</p>
              </div>
            )}
            {!processed.remarks && !processed.teacher_feedback && (
              <p className="text-center text-slate-500 py-8">No additional feedback available</p>
            )}
          </div>
        )}
...
// Result Card Component (display in grid)
const ResultCard = ({ result, childName, onViewDetails, onPrintCard }) => {
  const processed = processResult(result);
  const percentage = processed.overallPercentage;
  const examType = EXAM_TYPES[processed.exam_type] || EXAM_TYPES.monthly;
  const TypeIcon = examType.icon;
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      <div className={`bg-gradient-to-r p-5 ${
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <h3 className="text-xl font-extrabold text-white mt-1">{processed.exam_name}</h3>
            <div className="flex items-center gap-3 mt-2">
              <span className="text-sm text-white/80">
                📅 {format(new Date(processed.exam_date), 'dd MMM yyyy')}
              </span>
              {processed.rank && (
                <span className="text-sm text-white/80">
                  🏆 Rank #{processed.rank}
          ...
          <div className="text-center bg-white/20 rounded-xl px-4 py-3">
            <p className="text-3xl font-extrabold text-white">{percentage}%</p>
            <p className="text-xs text-white/70 mt-0.5">
              {processed.obtained_marks}/{processed.total_marks}
            </p>
            <span className="mt-1 inline-block text-xs font-bold px-2 py-0.5 rounded-full bg-white/20 text-white">
              Grade {processed.overallGrade}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="p-5">
        ...
        <div className="space-y-2.5">
          {processed.subjects.slice(0, 3).map((s, idx) => {
        ...
        {processed.subjects.length > 3 && (
          <p className="text-xs text-slate-400 mt-3 text-center">
            +{processed.subjects.length - 3} more subjects
          </p>
        )}
      </div>
    </div>
...
const ResultCardModal = ({ isOpen, onClose, result, child, institute }) => {
  ...
  const { student, exam, result: cardResult } = transformForPrintableCard(processedResult, child, institute);
...
export default function ParentResultsPage() {
  ...
  const rawResults = resultsData?.data?.results || [];
  // Process each result to add computed fields
  const results = rawResults.map(processResult);
  const statistics = statisticsData?.data;
  const upcomingExams = resultsData?.data?.upcoming_exams || [];
  ...
  const uniqueExamTypes = [...new Set(rawResults?.map(r => r.exam_type))].filter(Boolean);
  const examTypeOptions = [
    { value: 'all', label: 'All Exam Types' },
    ...uniqueExamTypes.map(type => ({
      value: type,
