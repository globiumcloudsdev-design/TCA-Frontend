...
function InstituteFormModal({ open, onClose, institute, onSubmit, loading, typeOptions }) {
  ...
  // ── Auto-fill trial_days when plan changes ─────────────────────────────────
  ...
  useEffect(() => {
    ...
    setValue('trial_days', days, { shouldDirty: true });
    const d = new Date();
    d.setDate(d.getDate() + days);
    setValue('trial_end_date', d.toISOString().split('T')[0], { shouldDirty: true });
  }, [selectedPlanId, planOptions.length]);

  // ── Auto-compute trial_end_date when trial_days changes manually ───────────
  const trialDays = watch('trial_days');
  useEffect(() => {
    const d = parseInt(trialDays);
    if (!d || d <= 0 || selectedPlanId) return;
    const end = new Date();
    end.setDate(end.getDate() + d);
    setValue('trial_end_date', end.toISOString().split('T')[0]);
  }, [trialDays]);
    ...
    >
      <form onSubmit={handleSubmit(handleInternalSubmit)} className="space-y-0.5">
        ...
        <SelectField
          ...
          hint="Selecting a plan automatically fills trial days and end date"
        />
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <InputField
            label="Trial Days" name="trial_days" register={register} type="number"
            placeholder="30" hint="Auto-filled when you select a plan"
          />
          <DatePickerField
            label="Trial End Date" name="trial_end_date" control={control}
            placeholder="Pick end date"
            hint="Auto-computed from trial days"
          />
          <SelectField
