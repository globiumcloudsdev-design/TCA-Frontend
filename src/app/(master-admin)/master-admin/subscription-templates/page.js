...
import {
  ...
  Users, GraduationCap, GitBranch, HardDrive, RefreshCw,
  Bug, ChevronDown, ChevronUp, Eye, Power, Flame,
  X, Clock, Package, WifiOff, Loader2,
} from 'lucide-react';
import { toast } from 'sonner';
...
// Extended feature list to match the product comparison table
const FEATURE_DEFS = [
  // Core
  { key: 'student_management',   label: 'Student Management',   group: 'Core' },
  { key: 'teacher_management',   label: 'Teacher Management',   group: 'Core' },
  ...
  { key: 'exam_management',      label: 'Exam & Results',       group: 'Core' },
  { key: 'fee_management',       label: 'Fee Management',       group: 'Core' },
  { key: 'parent_teacher_chat',  label: 'Parent ↔ Teacher Chat', group: 'Core' },
  ...
  { key: 'hostel_management',    label: 'Hostel',               group: 'Advanced' },
  { key: 'inventory_management', label: 'Inventory',            group: 'Advanced' },
  { key: 'student_accounting',   label: 'Student Accounting',   group: 'Advanced' },
  { key: 'office_accounting',    label: 'Office Accounting',    group: 'Advanced' },
  // { key: 'certificate_management', label: 'Certificate Management', group: 'Advanced' },
  // { key: 'card_management',      label: 'Card Management',      group: 'Advanced' },
  // { key: 'alumni_management',    label: 'Alumni Management',    group: 'Advanced' },
  ...
  // White-label & Document Management
  { key: 'document_management',  label: 'Document Management',  group: 'White‑label' },
  { key: 'custom_school_app',    label: 'Custom School App',    group: 'White‑label' },
  { key: 'play_store_listing',   label: 'Play Store / App Store Listing', group: 'White‑label' },
  { key: 'white_label_branding', label: 'School Name Branding', group: 'White‑label' },
];
...
// Helper to build default feature object (all false)
function buildEmptyFeatures() {
  const empty = {};
  FEATURE_DEFS.forEach(({ key }) => { empty[key] = false; });
  return empty;
}

// ─── Pre‑defined Templates (Starter, Growth, Pro, School Method) ─────────────
// First, define the Pro features separately to avoid self‑reference
const PRO_FEATURES = {
  ...
  // White‑label features are off in Pro
  document_management: false,
  custom_domain: false,
  custom_school_app: false,
  play_store_listing: false,
...
const TEMPLATES = {
  starter: {
    name: 'Starter',
    code: 'STARTER',
    description: 'Essential ERP for small schools – core modules only',
    cycle: 'MONTHLY',
    price: 3999,
    currency: 'PKR',
    trial_days: 14,
    ...
    features: {
      ...
      ...buildEmptyFeatures(),
      // override only the ones we want true
      student_management: true,
      teacher_management: true,
      parent_portal: true,
      attendance_system: true,
  ...
  growth: {
    name: 'Growth',
    code: 'GROWTH',
    description: 'Advanced ERP with multi‑branch support & parent chat',
    cycle: 'MONTHLY',
    price: 6999,
    currency: 'PKR',
    trial_days: 14,
  ...
  pro: {
    name: 'Pro',
    code: 'PRO',
    description: 'Complete ERP – all modules, unlimited branches',
    cycle: 'MONTHLY',
    price: 11999,
    currency: 'PKR',
    trial_days: 30,
  ...
  schoolMethod: {
    name: 'School Method (White‑Label)',
    code: 'WHITELABEL',
    description: 'Your own branded school system – custom app & store listing',
    cycle: 'MONTHLY',
    price: 49999,
    currency: 'PKR',
    trial_days: 30,
...
function planToForm(plan = {}) {
  ...
  // Default features – enable a few sensible ones for new plan
  const defaultFeatures = buildEmptyFeatures();
  defaultFeatures.student_management = true;
  defaultFeatures.teacher_management = true;
  defaultFeatures.parent_portal = true;
  defaultFeatures.attendance_system = true;
  defaultFeatures.exam_management = true;
  defaultFeatures.fee_management = true;
  defaultFeatures.email_notifications = true;
  defaultFeatures.basic_reports = true;
  defaultFeatures.payment_gateway = true;

  if (!plan.id) {
    return {
      ...d,
      ...Object.fromEntries(
        FEATURE_DEFS.map(({ key }) => [`f_${key}`, defaultFeatures[key] ?? false])
      ),
    };
  }
  ...
  return {
    ...
    storage_gb:    plan.limits?.storage_gb   ?? d.storage_gb,
    ...Object.fromEntries(
      FEATURE_DEFS.map(({ key }) => [`f_${key}`, plan.features?.[key] ?? defaultFeatures[key] ?? false])
    ),
  };
}
...
function formToBody(fv) {
  const features = {};
  FEATURE_DEFS.forEach(({ key }) => { features[key] = !!fv[`f_${key}`]; });
...
// Helper to convert a template object to form values (prefix features with f_)
function templateToForm(template) {
  const form = {
    ...
    max_teachers: template.max_teachers,
    max_staff: template.max_staff,
    max_admins: template.max_admins,
    storage_gb: template.storage_gb,
  };
  // add feature flags
  FEATURE_DEFS.forEach(({ key }) => {
    form[`f_${key}`] = template.features[key] ?? false;
  });
  return form;
}
...
function PlanCard({ plan, idx, toggling, onEdit, onDelete, onToggle }) {
  ...
  return (
    <div className={cn(
      ...
      <div className="flex gap-2 mt-auto">
        <Button size="default" variant="outline" className="flex-1 gap-2 text-sm h-9 sm:h-8" onClick={onEdit}>
          <Pencil size={14} /> Edit
        </Button>
        <Button
          size="default"
...
// ─── Plan Form Modal (with Quick Templates) ───────────────────────────────────
...
function PlanFormModal({ open, onClose, plan, onSubmit, loading }) {
  ...
  // Apply a predefined template to the form
  const applyTemplate = (templateKey) => {
    const template = TEMPLATES[templateKey];
    if (template) {
      const formValues = templateToForm(template);
      reset(formValues);
      toast.success(`"${template.name}" template loaded – review & save`);
    }
  };
      ...
      <div className="mb-5 p-3 bg-blue-50/50 rounded-lg border border-blue-100">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <span className="text-xs font-semibold text-blue-700 flex items-center gap-1">
            <Star size={12} /> Quick Templates
          </span>
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => applyTemplate('starter')}>
              Starter (Rs 3,999)
            </Button>
            <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => applyTemplate('growth')}>
              Growth (Rs 6,999)
            </Button>
            <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => applyTemplate('pro')}>
              Pro (Rs 11,999)
            </Button>
            <Button size="sm" variant="outline" className="h-7 text-xs border-purple-300 bg-purple-50" onClick={() => applyTemplate('schoolMethod')}>
              🏫 School Method (White‑Label)
            </Button>
          </div>
        </div>
        <p className="text-[11px] text-muted-foreground mt-2">
          Click any template to pre‑fill all fields. Then adjust if needed and save.
        </p>
      </div>
